...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: mvp_scope
      decision: pełny flow z notatek; zawężenie v1 — minimalna auth (bez resetu hasła / weryfikacji e-mail); mvp_weeks 3
    - topic: access_model
      decision: logowanie e-mail + hasło; płaski model — jeden typ użytkownika, właściciel własnych projektów
    - topic: context_type
      decision: greenfield — brak istniejącego kodu w repozytorium
    - topic: pain_category
      decision: tarcie w workflow — drogie, przeładowane oprogramowanie podnosi barierę wejścia w obliczenia ciepła budynku
    - topic: primary_persona
      decision: pojedynczy wykonawca / projektant instalacji lub audytor energetyczny
    - topic: insight
      decision: cena + prostota UI wygrywają z „wszystkim w jednym” u liderów rynku dla użytkownika, który potrzebuje OZC na rzucie, nie pełnego CAD/BIM
    - topic: target_scale_qps
      decision: low — sporadyczne obliczenia, kilku użytkowników (rozstrzygnięte przy review PRD 2026-05-19)
    - topic: target_scale_data_volume
      decision: small — kilka–kilkanaście projektów, pojedyncze PDF-y rzutów (rozstrzygnięte przy review PRD 2026-05-19)
    - topic: calculation_standard
      decision: WT 2021 — straty przez przegrody; wentylacja grawitacyjna per pomieszczenie — model uproszczony MVP (rozstrzygnięte przy review PRD 2026-05-19)
    - topic: primary_flow_step2
      decision: projekt od nazwy; rzut przy imporcie PDF w edytorze — zgodnie z FR-002 / FR-007 (rozstrzygnięte przy review PRD 2026-05-19)
  frs_drafted: 9
...
**Koszt dziś:** Drogi, przeładowanym funkcjami program obliczeniowy lub rozdzielony workflow (rzut poza narzędziem obliczeniowym), wysoka bariera przed pierwszym sensownym wynikiem.

## Access Control

Logowanie: e-mail + hasło. Rejestracja konta użytkownika.

Model dostępu: płaski — jeden typ użytkownika; każdy użytkownik jest właścicielem własnych projektów. Brak ról admin/członek/gość w MVP. Brak współdzielenia projektu między kontami (zgodnie z non-goals w notatkach).
...
8. Uruchamia obliczenie i widzi na ekranie straty na ciepło oraz wentylację zgodne z oczekiwaniem inżynierskim (bez formalnego dokumentu raportu w v1).
...
Aplikacja oblicza stratę ciepła budynku i zapotrzebowanie wentylacyjne z geometrycznych stref pomieszczeń na rzucie, zdefiniowanych przegród, parametrów klimatycznych oraz wentylacji grawitacyjnej per pomieszczenie; straty przez przegrody odniesione do **WT 2021**.
...
Wszystkie elementy bramki jakości obecne na zakończenie sesji (greenfield).

## Open Questions (resolved)

Rozstrzygnięte 2026-05-19 przy review PRD — zsynchronizowane z `context/foundation/prd.md`:

- `target_scale.qps`: **low**
- `target_scale.data_volume`: **small**
- Norma obliczeń: **WT 2021** (straty przez przegrody); wentylacja grawitacyjna per pomieszczenie — model uproszczony MVP.
